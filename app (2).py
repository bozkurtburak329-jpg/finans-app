"""
╔══════════════════════════════════════════════════════════════╗
║  MİDAS KLONU v12.0 · BİST KANTİTATİF ANALİZ & TERMİNALİ      ║
║  Canlı Kur, Hedef Fiyatlar, Basitleştirilmiş Grafikler       ║
╚══════════════════════════════════════════════════════════════╝
"""
...
def fetch_macro_data():
    ...
    def get_data(t):
        try:
            # 5 günlük veri çekilir, son geçerli gün (haftasonu bile olsa Cuma) alınır
...
def fetch_market_data():
    ...
    def process_ticker(ticker, name):
        try:
            ...
            sma50, rsi = float(close.rolling(50).mean().iloc[-1]) if len(close)>=50 else price, compute_rsi(close)
            _, _, macd_h = compute_macd(close)
            atr_val = (close.iloc[-1] - close.iloc[-2]) # Basit volatilite
            return {"Sembol": name, "Fiyat (TL)": price, "Degisim %": (price - prev) / prev * 100, 
                    "RSI": rsi, "MACD_H": macd_h, "Aksiyon": get_action_signal(rsi, price, sma50), "SMA50": sma50, "Volatilite": atr_val}
        except: return None
...
with tab_detail:
    ...
    if selected_ticker:
        ...
        if hist.empty or len(hist) < 5:
            st.warning(f"⚠️ {selected_symbol} için yeterli piyasa verisi bulunamadı. Hisse işlem görmüyor veya sembol güncellenmiş olabilir.")
        else:
            ...
            with col2:
                # KARMAŞIK OLMAYAN, SADE, PÜRÜZSÜZ GRAFİK
                fig = go.Figure()
            ...
            c4.markdown(f'<div class="bn-card"><div class="bn-card-label">RSI (14)</div><div class="bn-card-value" style="color:{"#00e676" if rsi_14<40 else "#ff3d00" if rsi_14>65 else "#fff"};">{rsi_14:.1f}</div></div>', unsafe_allow_html=True)
        ...
        with st.spinner("Piyasa taranıyor, hedef fiyatlar ve yıl sonu ihtimalleri hesaplanıyor..."):
            ...
            if df_scan.empty:
                ...
                for _, row in df_scan.iterrows():
                    ...
                    hedef_fiyat = fiyat * 1.15 # %15 kar hedefi
                    stop_loss = fiyat * 0.92 # %8 zarar kes
                    ...
                    <div style="background:#161A1E; border:1px solid #2B3139; border-left:4px solid #0ECB81; border-radius:8px; padding:1.5rem; margin-bottom:1rem;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
                            ...
                            <span style="font-size:1.2rem; font-weight:600; color:#0ECB81;">Şu anki Fiyat: ₺{fiyat:,.2f}</span>
                        </div>
